...
- **SmartArt with no drawing cache — non-hList/hier layouts.** Wave 10 G (`feat/w10g-smartart-layout`) added a minimal layout engine for horizontal/vertical lists and tree hierarchies. Cycle, matrix, radial, and pyramid templates still render as a `[SmartArt]` placeholder when `diagrams/drawingN.xml` is absent. A full ECMA-376 §21.4 engine (~20 algorithms) is a multi-month project and deliberately out of scope.
...
- Diagonal cell borders on merged cells (Wave 10, C, `feat/w10c-table-gaps`). Verified via a browser probe of the basic fixture: a `colspan=2` cell with `<a:lnTlToBr>` renders an `<svg viewBox="0 0 100 100" preserveAspectRatio="none">` overlay whose line endpoints map (via `getScreenCTM`) exactly to the merged `<td>`'s top-left and bottom-right corners — i.e. the diagonal traces the full merged rectangle, not just the primary cell's original column. No code change needed; the existing per-cell overlay in `renderCell` already handles spans because `colSpan`/`rowSpan` on the `<td>` stretches the SVG. The `presentation.pptx` fixture now includes the regression case on a dedicated slide, and `renderCell` carries a comment pointing future readers at this. Continuation-cell diagonals (hMerge/vMerge-hidden cells that themselves declared a diagonal in the unmerged layout) are still dropped — PowerPoint itself doesn't produce those in practice.
- Per-master tableStyles — closed as a spec non-feature (Wave 10, C, `feat/w10c-table-gaps`). Re-reading ECMA-376 Part 1 §14.2.9, the `tableStyles` relationship type is declared only on the PresentationML Presentation part; the SlideMaster content type has no corresponding relationship. OOXML has no "per-master tableStyles" to support. The original TODO entry was a misread of the spec. `presentation.ts` now carries a comment alongside `pres.tableStyles` spelling this out.
- `<p14:sectionLst>` UI affordances — demo sections drawer (Wave 6, A5, `feat/w6a5-sections-drawer`, commit 97bf6c2 "feat(demo): sections drawer populated from presentation.sections"). Collapsible left drawer on the demo page lists each section with its 1-based slide range and click-to-scroll into the corresponding `<section data-slide-index>`. An `IntersectionObserver` rooted on the scrolling container flips the active-section highlight as slides cross the viewport. The drawer is hidden via `d-none` when the deck has no sections. `scripts/make-fixture-basic.py` splices a `<p14:sectionLst>` under `<p:extLst>` with four named sections so the basic fixture exercises both the parser and the drawer; a Wave 10 F audit regenerated the committed `.pptx` (it had drifted after an intermediate fixture regeneration in commit 54050bb silently dropped the splice) and added `tests/render-test/basic/sections.spec.js` as a regression guard asserting section names, indices, and full coverage.
- Picture-renderer omnibus (Wave 10, E, `feat/w10e-pic-omnibus`). Three fidelity bumps on `src/render/pic.ts` + `src/render/fill-utils.ts`: (1) `applyTileMetrics` probes the source image's `naturalWidth`/`naturalHeight` via an `Image()` load and patches `backgroundSize` / `backgroundPosition` in place, honouring `<a:tile tx ty sx sy>` in source-pixel units; `algn` (tl/t/tr/l/ctr/r/bl/b/br) applies synchronously and overrides tx/ty. `flip="x|y|xy"` is noted as a TODO (no CSS analogue). (2) `duotone` upgraded from `mix-blend-mode: multiply` to an inline SVG `feColorMatrix` (Rec. 601 luminance) + `feComponentTransfer` (two-entry tableValues for the dark→light ramp); blend-mode fallback preserved when `supportsSvgFilters()` is false. (3) `biLevel` upgraded from `grayscale + brightness + contrast(1000)` to a luminance `feColorMatrix` + `feComponentTransfer type="linear"` with slope `1e6` and `intercept = -slope * threshold`, producing a crisp step at the per-mille threshold; CSS approximation kept as fallback.
- Minimal SmartArt layout engine (Wave 10, G, `feat/w10g-smartart-layout`). When `diagrams/drawingN.xml` is absent, `src/smartart-layout.ts` now parses `diagrams/data1.xml` + `diagrams/layout1.xml` and synthesises shape boxes for horizontal/vertical lists (`hList*`, `vList*`) and tree hierarchies (`hierRoot*`, `orgChart*`). All other templates (cycle, matrix, radial, pyramid, etc.) still render as the `[SmartArt]` placeholder — a full ECMA-376 §21.4 engine is deliberately out of scope.
